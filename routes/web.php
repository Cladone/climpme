...
Route::middleware(['auth','verified'])->group(function () {
    Route::get('/traffic', [TrafficReportController::class, 'overview'])->name('traffic.index');
    Route::get('/traffic/top', [TrafficReportController::class, 'show'])->name('traffic.show');
    Route::get('/traffic/post/{postId}', [TrafficReportController::class, 'showPost'])->name('traffic.post');

    Route::get('/traffic/referrers',  [TrafficReportController::class, 'referrers'])->name('traffic.referrers');
    Route::get('/traffic/countries',  [TrafficReportController::class, 'countries'])->name('traffic.countries');
    Route::get('/traffic/browsers',   [TrafficReportController::class, 'browsers'])->name('traffic.browsers');
    Route::get('/traffic/os',         [TrafficReportController::class, 'os'])->name('traffic.os');
});

// routes/web.php
Route::get('/traffic/health', [\App\Http\Controllers\TrafficReportController::class, 'health'])->middleware(['auth','verified'])->name('traffic.health');
// routes/web.php
Route::get('/debug/headers', function (\Illuminate\Http\Request $r) {
    return response()->json([
        'cf_ipcountry'      => $r->header('CF-IPCountry'),
        'cf_connecting_ip'  => $r->header('CF-Connecting-IP'),
        'ip_seen_by_laravel'=> $r->ip(),
        'host'              => $r->getHost(),
    ]);
})->middleware(['auth']);
...
/*
|--------------------------------------------------------------------------
| QR Routes (library, create, builder, generate, download, view)
| Wrapped with 'verified' to match other app pages.
| id is numeric to avoid conflicts with strings like 'new' or 'builder'.
|--------------------------------------------------------------------------
*/
Route::middleware('verified')->group(function () {
    // QR Library (top-level list)
    Route::get('/qr', 'QrController@library')->name('qr.index');

    // Create from scratch (page + submit)
    Route::get('/qr/new', 'QrController@create')->name('qr.create');
    Route::post('/qr', 'QrController@store')->name('qr.store');

    // Builder + (re)generate + download
    Route::get('/qr/{id}/builder',  'QrController@builder')->whereNumber('id')->name('qr.builder');
    Route::post('/qr/{id}/generate','QrController@generate')->whereNumber('id')->name('qr.generate');
    Route::get('/qr/{id}/download', 'QrController@download')->whereNumber('id')->name('qr.download');

    // Existing quick page (optional)
    Route::get('/qr/{id}', 'QrController@index')->whereNumber('id')->name('qr');
});
...
/*
|--------------------------------------------------------------------------
| Short-link Redirect routes (catch-all) – exclude reserved prefixes
|--------------------------------------------------------------------------
*/
$reserved = 'qr|admin|install|update|account|pricing|checkout|developers|dashboard|links|spaces|domains|pixels|stats|contact|pages|traffic';

Route::get('/{id}/+', 'RedirectController@index')
    ->where('id', "^(?!($reserved)$).+")
    ->name('link.preview');
