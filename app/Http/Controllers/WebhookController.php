        ...
        if (hash_equals($computedSignature, $signatureParameter[1])) {
            try {
                $paddle = new \Paddle\SDK\Client(
                    apiKey: config('settings.paddle_api_key'),
                    options: new \Paddle\SDK\Options((config('settings.paddle_mode') == 'sandbox' ? \Paddle\SDK\Environment::SANDBOX : \Paddle\SDK\Environment::PRODUCTION)),
                );
            } catch (\Exception $e) {
                Log::info($e->getMessage());
